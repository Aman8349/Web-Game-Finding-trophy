...
class Player {
  ...
  draw() {
    ctx.save();
    ctx.translate(
      this.x + this.scaleWidth * 0.5,
      this.y + this.scaleHeight * 0.5
    );
  ...
  setKeyDirection(direction) {
    if (direction === "south") this.angle = 0;
    else if (direction === "west") this.angle = 90;
    else if (direction === "north") this.angle = 180;
    else if (direction === "east") this.angle = -90;
  }
...
/**levelMap
 * levelMap[index].world is the virtual islands where game component lives in
 * levelMap[index].number is the current level's id
 * levelMap[index].title is the name of the current level
 */
const levelMap = [
  {
    number: 1,
...
/**tile
 * main tilesheet of the game, saved locally as
 * "/home/mirielle/games/web/tilebased/defend.png"
 * saved to the cloud as
 * "https://i.ibb.co/hDK9Qyz/defend.png"
 */
const tile = {
  img: new Image(),
...
const update = () => {
  ...
  if (
    Math.abs(new Date().getMinutes() - lastPlayed.getMinutes()) >=
    4 * (level + 1)
  ) {
    animation.endGame();
  }
  ...
  let timeRemainder = new Text(
    "You are given " + Number(5 * (level + 1)) + " Minutes",
    W / 2,
    30,
...
const mainScreen = () => {
  ...
  if (splashScreenLoaded) {
    ctx.fillStyle = util.colors.sea;
    util.drawRect([0, 0, W, H / 2], ["lightgray", "rgba(0, 0, 0, .5)", 4, 4]);
    ...
    util.drawRect([0, H - 65, W, 65], ["lightgray", "rgba(0, 0, 0, .5)"], 4, 4);
    let startGame = new Text(
      "Start",
      W / 2,
      H - 35,
...
const eventHandler = () => {
  ...
  window.addEventListener("keydown", (e) => {
    if (e.keyCode === 37) canMove.west();
    else if (e.keyCode === 39) canMove.east();
    else if (e.keyCode === 38) canMove.north();
    else if (e.keyCode === 40) canMove.south();
  });
...
const canMove = {
  ...
  north() {
    ...
    if (mazeIndex === 45) animation.newLevel();
  },
  ...
  south() {
    ...
    let posY =
      ...
      camera.speed +
      camera.y -
      (tile.scaleHeight - player.scaleHeight);
    let col = ~~(posX / tile.scaleWidth);
  ...
  east() {
    let posX =
      ...
      camera.speed +
      camera.x -
      (tile.scaleWidth - player.scaleWidth);
    let posY = player.y + camera.y;
